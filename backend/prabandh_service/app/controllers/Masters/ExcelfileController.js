...
routers.get("/downloadfile/:id/:user_state_id", async (req, res) => {
  ...
  if (id.includes("ticket")) {
    ...
    let excelFilePath = path.join(UPLOAD_FILE_PATH, `/ticket/${ticket?.file}`);
    excelFilePath = path.resolve(excelFilePath);
    ...
    if (!filename.state_id) {
      excelFilePath = path.join(UPLOAD_FILE_PATH, `/uploads/state/0/${filename?.file_name}`);
      excelFilePath = path.resolve(excelFilePath);
    } else {
      excelFilePath = path.join(UPLOAD_FILE_PATH, `/uploads/state/${filename.state_id}/${filename?.file_name}`);
      excelFilePath = path.resolve(excelFilePath);
    }
    res.sendFile(excelFilePath, (err) => {
      if (err) {
