...
exports.find = async (req, res) => {
    ...
    try {
        ...
        if (object != null) {
            let dietmonthData = await Model.knx().raw(`select * from progress.diet_progress_active_month where state_id=${req.body.district_state_id} and plan_year='${apiYear}'`);
        ...
        //    res.status(200).json({ status: true, 'data': object });
            return Response.handle(req, res, 'updateStatus', 200, { status: true, 'data': object, diet_progress_month })
        } else {
        //    res.status(404).json({ status: false, message: Message.notFound('Record') });
            return Response.handle(req, res, 'updateStatus', 404, { status: false, message: Message.notFound('Record') })
        }
