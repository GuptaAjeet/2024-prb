...
exports.index = async(req,res) =>{
    try{
        ...
        const object  = await Model.query().select(['id','status_name','status_order']).orderBy('status_name').
...
exports.detailedList = async(req,res) =>{
    try{
        const object = await Model.detailedList()
        res.status(200).json({status:true,'data':object}); 
    }catch(e){
        return Exception.handle(e,res,req,'');
    }
