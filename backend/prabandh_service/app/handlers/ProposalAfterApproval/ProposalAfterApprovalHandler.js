...
exports.schoolList = async (req, res) => {
  try {
    ...
    let whereDistrict = '';
    if(district_id){
      whereDistrict = `and psm.district_id = ${district_id}`
    }
    ...
    let schoolData = await Model.knx()
      ...
      ${whereDistrict} order by school_name`);
...
exports.financilaSchoolList = async (req, res) => {
  try {
    const { data, state_id, district_id, diet } = req.body;
    const apiYear = req.headers?.api_year;
    ...
    let schoolData = await Model.knx()
      .raw(`select distinct psm.udise_sch_code , school_name , psm.district_id 
      from allocation.prb_ann_wrk_pln_bdgt_data_allocation_asset pawpbdaa , prb_school_master psm  
      where pawpbdaa.state_id = ${state_id} and pawpbdaa.plan_year ='${apiYear}' ${diet === true ? "and sub_component_id = 222" : ""}
      and pawpbdaa.asset_code = psm.udise_sch_code 
      ${whereDistrict} order by school_name`);
...
exports.schoolListOfBlock = async (req, res) => {
  try {
    ...
    //   .raw(`select distinct psm.udise_sch_code , school_name 
    //   from allocation.prb_ann_wrk_pln_bdgt_data_allocation_asset pawpbdaa , prb_school_master psm  
    //   where pawpbdaa.state_id = psm.state_id 
    //   and pawpbdaa.plan_year ='${apiYear}'
    //   and pawpbdaa.asset_code = psm.udise_sch_code 
    //   and psm.block_cd = '${block_cd}' order by school_name`);

    let schoolData = await Model.knx().raw(`select * from prb_school_master psm where psm.block_cd = '${block_cd}'`);
      ...
      activityList = await Model.knx().raw(`with cte as (
          select prb_ann_wrk_pln_bdgt_data_progress_asset_id, asset_code, asset_type, 
             ...
             pd.scheme_name , pd.major_component_name , pd.sub_component_name , pd.activity_master_name , pd.activity_master_details_name ,
                 pawpbdpa.allocated_physical_quantity , pawpbdpa.allocated_financial_amount , 
                 physical_progress_yet_to_start, physical_progress_in_progress, physical_progress_completed,financial_expenditure,
                 state_id , district_id , plan_year,month_id , status
           from progress.prb_ann_wrk_pln_bdgt_data_progress_asset pawpbdpa , prb_data pd 
           where 
...
exports.dietConfigctivity = async (req, res) => {
  try {
    ...
    const diet_progress_month = req.headers?.diet_progress_month;

    await Model.knx().raw(`INSERT INTO progress.diet_progress_plan
    (diet_id,state_id, district_id, plan_year, scheme_id, major_component_id, sub_component_id, activity_master_id, activity_master_details_id,   status,month_id, created_by)
    select md.diet_id ,md.district_state_id , md.district_id ,'${apiYear}',  pd.scheme_id , pd.major_component_id , pd.sub_component_id , pd.activity_master_id , pd.id,1,${diet_progress_month} ,${created_by}
    from prb_data pd , master_diet md 
    where sub_component_id = 222 and  diet_id = ${diet_id} on conflict(diet_id,state_id,district_id ,activity_master_details_id,plan_year,month_id)
    do nothing`);
    ...
    let object = await Model.knx().raw(`select 
      ...
      where diet_id = ${diet_id} and plan_year ='${apiYear}' and state_id=${state_id} and month_id=${diet_progress_month}
      ...
      activityList = await Model.knx().raw(`with cte as (
          select '${asset_code}' as asset_code, 5 as asset_type, 
              ...
              pd.scheme_name , pd.major_component_name , pd.sub_component_name , pd.activity_master_name , pd.activity_master_details_name ,
                  pawpbdaa.allocated_physical_quantity , pawpbdaa.allocated_financial_amount , 
                  0 as physical_progress_yet_to_start, 0 as physical_progress_in_progress, 0 as physical_progress_completed,0 as financial_expenditure,
                  state_id , district_id , plan_year, ${diet_progress_month} as month_id, 1 as status 
          from allocation.prb_ann_wrk_pln_bdgt_data_allocation_asset pawpbdaa , prb_data pd 
          where asset_code ='${asset_code}' and plan_year = '${apiYear}' and state_id=${state_id} and district_id=${district_id}
          and pawpbdaa.activity_master_details_id = pd.id 
          ...
          progress.prb_ann_wrk_pln_bdgt_data_progress_asset pawpbdpa
          where 
          asset_code ='${asset_code}'  and plan_year = '${apiYear}'  and month_id=${diet_progress_month}
          group by asset_code , activity_master_details_id 
          )
          ...
          LEFT JOIN 
            ...
            and dpp.plan_year = cte.plan_year and dpp.month_id=${diet_progress_month} order by activity_master_details_name`);
    } else {
      activityList = await Model.knx().raw(`with cte as (
          select prb_ann_wrk_pln_bdgt_data_progress_asset_id, asset_code, asset_type, 
             pd.scheme_id , pd.major_component_id, pd.sub_component_id  , pd.activity_master_id , pawpbdpa.activity_master_details_id,
  ...
  try {
    ...
    let data = await Model.knx().raw(`with cte as (
      select 
            ...
            pawpbdpa.asset_code , activity_master_details_id, status
...
exports.activityAdditionalParams = async (req, res) => {
  try {
    const { activity_master_details_id } = req.body;
    const apiYear = req.headers?.api_year;

    // let data = await Model.knx().raw(`select * from progress.master_additional_parameter_progress where activity_master_details_id=${activity_master_details_id}`);

    let data = await Model.knx().raw(`select mapp.*, ap.additional_parameter_value from progress.master_additional_parameter_progress mapp
      left join progress.additional_parameter ap on ap.master_additional_parameter_progress_id = mapp.master_additional_parameter_progress_id
      where mapp.activity_master_details_id=${activity_master_details_id}`);

    return Response.handle(req, res, "activityAdditionalParams", 200, {
      status: true,
      data: data.rows,
    });
  } catch (e) {
    return Exception.handle(e, res, req, "activityAdditionalParams");
  }
};


exports.saveAdditionalParams = async (req, res) => {
  try {
    const { data } = req.body;
    const apiYear = req.headers?.api_year;

    let afterInsert = await Model.knx().raw(`INSERT INTO progress.additional_parameter
      (prb_ann_wrk_pln_bdgt_data_progress_asset_id, master_additional_parameter_progress_id, additional_parameter_value, asset_code, activity_master_details_id, plan_year)
       VALUES ${data.map(item => `(${item.prb_ann_wrk_pln_bdgt_data_progress_asset_id}, ${item.master_additional_parameter_progress_id}, '${item.additional_parameter_value}', '${item.asset_code}', ${item.activity_master_details_id}, '${apiYear}')`).join(',')} 
       on conflict (prb_ann_wrk_pln_bdgt_data_progress_asset_id,master_additional_parameter_progress_id,activity_master_details_id, plan_year) do 
        update set additional_parameter_value = excluded.additional_parameter_value`);

    return Response.handle(req, res, "saveAdditionalParams", 200, {
      status: true,
      data: afterInsert,
    });
  } catch (e) {
    return Exception.handle(e, res, req, "saveAdditionalParams");
  }
};


exports.lockByActivity = async (req, res) => {
  try {
    const { district_id, activity_master_details_id, prb_ann_wrk_pln_bdgt_data_progress_asset_id } = req.body;
    const apiYear = req.headers?.api_year;
    const user_district_id = req.auth.user.user_district_id;

    let status = user_district_id ? 3 : 6;
    
    let afterInsert;

    if(prb_ann_wrk_pln_bdgt_data_progress_asset_id){
      afterInsert = await Model.knx().raw(`update progress.prb_ann_wrk_pln_bdgt_data_progress_asset set status=${status}
      where plan_year='${apiYear}' and district_id=${district_id} and prb_ann_wrk_pln_bdgt_data_progress_asset_id=${prb_ann_wrk_pln_bdgt_data_progress_asset_id}`);
    }else{
      afterInsert = await Model.knx().raw(`update progress.prb_ann_wrk_pln_bdgt_data_progress_asset set status=${status}
      where plan_year='${apiYear}' and district_id=${district_id} and activity_master_details_id=${activity_master_details_id}`);
    } 

    return Response.handle(req, res, "lockByActivity", 200, {
      status: true,
      data: afterInsert,
    });
  } catch (e) {
    return Exception.handle(e, res, req, "lockByActivity");
  }
};
