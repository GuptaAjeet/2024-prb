...
exports.index = async (req, res) => {
  ...
  try {
    ...
    } else if (where.operator && (where.operator == ">" || where.operator == "<")) {
    ...
    let responseFromApiLogger = await ApiLog.create(token, req.originalUrl, "index", request, response);
...
exports.create = async (req, res) => {
  ...
  try {
    const userData = await Model.knx()
      ...
      .where(function () {
        this.where("au.user_email", request.email).orWhere(
          "au.user_mobile",
          request.mobile
        );
      });
    ...
    if (userData.length === 0) {
      ...
      // if(request.activity1.length > 0){
      //   request.activity1.forEach((item, index) => {
      //       Model.insertActivityDetails({user_id: data.id, role_id: request.role, activity_detail_id: item.id})
      //   })
      // }

      if (request.role == 16 || request.role == 17 || request.role == 23 || request.role == 24) {
        const iteratorForArray = Object.keys(request).filter((x) => x.startsWith("state"));
        const dbValues = [];
      ...
      responseFromApiLogger = await ApiLog.create(token, req.originalUrl, "create", request, { status: true, message: message });
      res.status(200).json({ status: true, message: message });
    } else {
      message = userData[0]?.condition_result;

      responseFromApiLogger = await ApiLog.create(token, req.originalUrl, "create", request, { status: false, message: message });
...
exports.update = async (req, res) => {
  ...
  try {
    ...
    END as condition_result`),
      ...
      .andWhere(function () {
        this.where("au.user_email", request.data.email).orWhere(
          "au.user_mobile",
          request.data.mobile
        );
      });
      ...
      if (result != null) {
        ...
        if (object) {
          await MailerHandler.updateUser(req, {
            subject: "Profile Update PRABANDH",
...
exports.updateStatus = async (req, res) => {
  ...
  try {
    ...
    if (result != null) {
      ...
      if (object) {
        await MailerHandler.updateUser(req, {
          subject: "Profile Status Update PRABANDH",
    ...
    const result = await Models.knx().raw(`select * from (
            ...
            FROM 
                public.consultant_mapping cm where user_id = ${user_id}
                    ...
                    and cc.idn = aa.id`);
    if (result != null) {
      responseFromApiLogger = await ApiLog.create(token, req.originalUrl, "userPer", req.body, { status: true, data: result.rows });
...
exports.profileUpdate = async (req, res) => {
  ...
  try {
    ...
    let responseFromApiLogger;
    if (result != null) {
      const user_name = request.data.name ? request.data.name : result.user_name;
      const user_email = request.data.email ? request.data.email : result.user_email;
      const user_mobile = request.data.mobile ? request.data.mobile : result.user_mobile;
      ...
      responseFromApiLogger = await ApiLog.create(token, req.originalUrl, "profileUpdate", request, { status: true, message: message, token: token });
      res.status(200).json({ status: true, message: message, token: token });
    } else {
      responseFromApiLogger = await ApiLog.create(token, req.originalUrl, "profileUpdate", request, { status: false, message: Message.notFound("Record") });
      res.status(200).json({ status: false, message: Message.notFound("Record") });
    }
  } catch (e) {
...
exports.findById = async (req, res) => {
  ...
  try {
    ...
    // data.activity_details_data = await Model.findActivityDetailsData({ user_id: id }) 
...
exports.dashboard = async (req, res) => {
  ...
  left join prb_major_component pmc 
    ...
    responseFromApiLogger = await ApiLog.create(token, req.originalUrl, "dashboard", req.body, response);
    res.status(200).json(response);
  } catch (e) {
...
exports.loggedUsersList = async (req, res) => {
  ...
  try {
    ...
    responseFromApiLogger = await ApiLog.create(token, req.originalUrl, "loggedUsersList", req.body, response);
    res.status(200).json(response);
  } catch (e) {
