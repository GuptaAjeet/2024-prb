...
const tableuradm = "user_role_activity_detail_mapping as uradm"
...
//   );
  ...
  let states = await PG.query(`select * from (select  min(status) , ms.state_id  , ms.state_name  , msm.pab_minute_status
    from prb_ann_wrk_pln_bdgt_data pawpbd , master_states ms left join report.master_states_minutes msm on ms.state_id = msm.state_id
    where state = ms.state_id and pawpbd.plan_year = '${apiYear}'
    group by ms.state_id  ,  ms.state_name, msm.pab_minute_status ) aa ${where} Order by state_name asc`);
...
exports.insertActivityDetails = async (object) => await DB(tableuradm).insert(object);
exports.findActivityDetailsData = async (object) => await DB(tableuradm).where(object).select();
