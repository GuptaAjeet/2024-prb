...
exports.validateToken = async (req, res) => {
  try {
    ...
    if (object[2] == "admin") {
      ...
      return res.status(404).json({ status: false, message: Message.unauthorized() });
    }
    if (object[2] == "school") {
      const user = await Model.School.findOne({
        udise_code: object[0],
  ...
  try {
    ...
    if (Crypto.decrypt(request.otc) == request.captcha) {
      ...
      if (request.flag == "admin") {
        ...
        return checkDuplicatePassword(request, res, user, Model.AdminUser, request.flag, (column = "user_password"), eq);
      }
      ...
      if (request.flag == "volunteer") {
        ...
        return checkDuplicatePassword(request, res, user, Model.Volunteers, request.flag, (column = "volunteer_password"));
      }
      // }
      // return res.status(200).json({status:false,message:Message.invalidOTP()});
    }
...
exports.changePassword = async (req, res) => {
  try {
    ...
    if (auth) {
      ...
      if (user) {
        return checkDuplicatePassword(request, res, user, Model.AdminUser, "admin", (column = "user_password"), req);
      } else {
        return res.status(200).json({ status: false, message: Message.notFound("User") });
      }
    } else {
      return res.status(200).json({ status: false, message: Message.unauthorized() });
    }
  } catch (e) {
    return Exception.handle(e, res, req, "changePassword");
  }
};
...
exports.checkUser = async (req, res) => {
  try {
    ...
    if ((object.flag == "admin" || object.flag == "volunteer") && object.user.user_mobile.length > 0) {
      return res.status(200).json(Library.OTPMaker.send(object.user.user_mobile));
    }
    if (object.flag == "school" && object.user.school_mobile.length > 0) {
      return res.status(200).json(Library.OTPMaker.send(object.user.school_mobile));
    }
  ...
  try {
    ...
    if (Crypto.decrypt(request.otc) == request.captcha) {
      ...
      if (request.flag == "admin") {
        ...
        return checkDuplicatePassword(request, res, user, Model.AdminUser, request.flag, "user_password");
      }
      if (request.flag == "school") {
        const user = await Model.School.findOne({
        ...
        return checkDuplicatePassword(request, res, user, Model.School, request.flag, "school_password");
      }
      if (request.flag == "volunteer") {
        const user = await Model.Volunteers.findOne({
        ...
        return checkDuplicatePassword(request, res, user, Model.Volunteers, request.flag, "volunteer_password");
      }
      // }
      // return res.status(200).json({status:false,message:Message.invalidOTP()});
    }
...
const checkDuplicatePassword = async (request, res, user, Table, flag, column, req) => {
  if (user != null) {
    ...
    if (OLD_PASS > 0) {
      return res.status(200).json({ status: false, message: Message.passwordUsed() });
    } else {
      ...
      return res.status(200).json({ status: true, message: Message.updated("Password") });
    }
  }
  return res.status(404).json({ status: false, message: Message.unauthorized() });
};