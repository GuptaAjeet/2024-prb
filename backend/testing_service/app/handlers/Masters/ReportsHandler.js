  ...
  try {
    ...
    let object = await Model.knx().raw(`select ps.title as scheme_name, pmc.title as major_component_name, psc.title as sub_component_name, 
      ...
      SELECT CAST(SUM(pawpbd.financial_amount) AS numeric(16, 5)) AS financial_amount, sum(pawpbd.physical_quantity) AS physical_quantity,
          (sum(pawpbd.financial_amount)/ nullif(sum(pawpbd.physical_quantity), 0))::numeric(16, 5) as unit_cost, pawpbd.scheme_id,
          pawpbd.major_component_id, pawpbd.sub_component_id, pawpbd.activity_master_id, pawpbd.activity_master_details_id
        ...
        (pawpbd.scheme_id, pawpbd.major_component_id, pawpbd.sub_component_id, pawpbd.activity_master_id), (pawpbd.scheme_id, pawpbd.major_component_id, 
          pawpbd.sub_component_id), (pawpbd.scheme_id, pawpbd.major_component_id), (pawpbd.scheme_id), ())
      ) 
      aa 
...
exports.stateCostingProposedSheetReport = async (req, res) => {
  try {
    ...
    case when (aa.activity_master_details_id is null) then 999999 else aa.activity_master_details_id end activity_master_details_id 
     from (
    SELECT  
        CAST(SUM(pawpbd.financial_amount) AS numeric(16, 5)) AS financial_amount, sum(pawpbd.physical_quantity) AS physical_quantity,
        ...
        max(coordinator_remarks)as coordinator_remarks, pawpbd.scheme_id, pawpbd.major_component_id, pawpbd.sub_component_id, pawpbd.activity_master_id,
...
exports.getSavedDataSpillFun = async (filters) => {
  ...
  if (state_id) {
    ...
    object = await Model.knx().raw(`select ROW_NUMBER() OVER () AS index,
...
exports.getSpilloversecond = async (req, res) => {
  try {
    const { state_id } = req.body;
    let result = await Model.knx().raw(`select * from report.view_spillover_report_without_scheme_final where state = ${state_id}
      order by major_component_id, sub_component_id, activity_master_id, activity_master_details_id`)

    return Response.handle(req, res, "getSpilloversecond", 200, {
      status: true,
      message: true,
      data: result.rows,
    });
  } catch (e) {
    return Exception.handle(e, res, req, "getSpilloversecond");
  }
};
...
exports.getAnnexureReport = async (req, res) => {
  try {
    ...
    return Response.handle(req, res, "getAnnexureReport", 200, {
      status: true,
      message: true,
      data: result.rows,
    });
  } catch (e) {
    return Exception.handle(e, res, req, "getAnnexureReport");
  }
};