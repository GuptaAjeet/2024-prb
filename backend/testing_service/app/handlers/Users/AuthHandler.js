    ...
    if (Crypto.decrypt(request.otc) == request.captcha) {
      if (request.type === "mobile") {
        if (request.otp === "" && request.password !== "") {
          ...
          if (Crypto.decrypt(request.ots) == request.otp || request.otp == env.DEFL_OTP) {
            if (request.flag === "admin") {
              let user = await Model.AdminUser.findOne({ user_mobile: request.mobile });

              if (user?.user_role_id === 1 && user?.reference_user_id > 0) {
                const userdata = await Model.AdminUser.findOne({ "u.id": user?.reference_user_id });
          ...
          return res.status(200).json({ status: false, otp: true, message: Message.invalidOTP() });
        }
      } else {
        return userHandler(req, res);
      }
  ...
  try {
    ...
    let apiRes = await axios.post(
      ...
      { as: "asad" },
      {
        headers: {
          "Content-Type": "application/json",
          identity: (env.APP_ENV == 'production' ? "test" : "shawez003"),
        },
      }
    );
    ...
    if (apiRes.data && apiRes.data.status) {
      ...
      const check = await Model.AdminUser.knx().raw(`select * from school_users where udise_code='${data.userId}'`);

      if (check.rows.length === 0) {
        const afterCreate = await Model.AdminUser.knx().raw(`INSERT INTO school_users (udise_code, user_name, user_mobile, user_role_id)
          VALUES (?, ?, ?, 22)
          RETURNING *`,
        ...
        if (afterCreate.rows.length > 0) {
          let user = await Model.AdminUser.knx().raw(
            `select * from school_users su join prb_school_master psm on su.udise_code = psm.udise_sch_code where su.udise_code = '${data.userId}'`
          );
          if (!user.rows && user.rows.length == 0) {
            user = await Model.AdminUser.knx().raw(`select * from school_users su su.udise_code = '${data.userId}'`);
          }
          user = user.rows[0];
        ...
        const afterUpdate = await Model.AdminUser.knx().raw(`update school_users set updated_at='${updated_at}' where udise_code='${data.userId}'`);
      ...
      return res.status(200).json({ status: false, message: apiRes.data.errorDetails.message });
    } else {
...
exports.simulate = async (req, res) => {
  try {
    ...
    if (request.type === "mobile") {
      if (request.flag === "admin") {
        const user = await Model.AdminUser.findOne({ user_mobile: user_mobile });
        return loginResponse(user, req, res, "admin");
      }
      responseFromApiLogger = await ApiLog.create(token, req.originalUrl, "simulate", request, { status: false, otp: true, message: Message.invalidOTP() });
      return res.status(200).json({ status: false, otp: true, message: Message.invalidOTP() });
    } else {
      return userHandler(req, res);
    }
...
const userHandler = async (req, res) => {
  ...
  if (request.flag == "admin") {
    const object = request.type == "email" ? { user_email: request.email } : { user_mobile: request.mobile };
  ...
  if (request.flag == "volunteer") {
    const object = request.type == "email" ? { volunteer_email: request.email } : { volunteer_mobile: request.mobile };
      ...
      if (+reference_user_id === 0) {
        if (selfdata) {
          if (selfdata && selfdata?.user_role_id < 4) {
            ...
            return res.status(200).json({ code: 200, status: true, message: "Refrence ID is Updated" });
          } else {
            return res.status(400).json({ code: 400, status: false, message: "You are not Authorized" });
          }
        } else {
          return res.status(400).json({ code: 400, status: false, message: "Refrence/Self ID not Exist" });
        }
      } else {
        if (userdata && selfdata) {
          if (selfdata && selfdata?.user_role_id < 4) {
            ...
            // if (updated) {
            return res.status(200).json({ code: 200, status: true, message: "Reference ID is Updated" });
            // }
          } else {
            return res.status(400).json({ code: 400, status: false, message: "You are not Authorized" });
          }
        } else {
          return res.status(400).json({ code: 400, status: false, message: "Reference/Self ID not Exist" });
        }
      }
    }
    return res.status(400).json({ code: 400, status: false, captcha: true, message: Message.captcha() });
  } catch (er) {
  ...
  try {
    ...
    if (request.otp != req.ENV.DEFL_OTP) {
      ...
      if (user != null) {
        ...
        res.status(404).json({ status: false, message: Message.notFound("User") });
      }
    }
  } catch (e) {
    return Exception.handle(e, res, req, "");
