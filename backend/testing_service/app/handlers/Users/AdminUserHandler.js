  ...
  try {
    ...
    if (userData.length === 0) {
      ...
      const result = await Model.create({
        ...
        created_by,
        assigned_group_activity_id: request.assigned_group_activity_id,
      });
      ...
      if (request.role == 16) {
        const iteratorForArray = Object.keys(request).filter((x) => x.includes("actgrpref"));
        ...
        if (iteratorForArray && iteratorForArray.length > 0) {
          iteratorForArray.forEach((item, index) => {
            if (item == 'scactgrpref') {
              dbValues.push({
                user_role_id: 15,
                activity_group_code: request[item],
                location_group_code: request['sclocgrpref'],
                user_id: data.id,
              });
            } else {
              dbValues.push({
                user_role_id: item.startsWith("nfc") ? 2 : 3,
                activity_group_code: item.startsWith("nfc") ? request[`nfcactgrpref${item.charAt(item.length - 1)}`] : request[`nicactgrpref${item.charAt(item.length - 1)}`],
                location_group_code: item.startsWith("nfc") ? request[`nfclocgrpref${item.charAt(item.length - 1)}`] : request[`niclocgrpref${item.charAt(item.length - 1)}`],
                user_id: data.id,
              });
            }
          });

          const grpMappingResult = await Model.createGrpMapping(dbValues);
        }
      }

      if (request.role == 17 || request.role == 23 || request.role == 24) {
        ...
        if (iteratorForArray && iteratorForArray.length > 0) {
          ...
          const consultantMappingResult = await ConsultantModel.create(dbValues);
        }
      }
  ...
  try {
    ...
    if (userData.length === 0) {
      ...
      if (result != null) {
        ...
        const assigned_group_activity_id = request.data && request.data.assigned_group_activity_id ? request.data.assigned_group_activity_id : result.assigned_group_activity_id;
        //const user_block_id     =   (request.data.block) ? request.data.block :result.user_block_id;
        const updated_by = req.auth.user.id;
        const update_data = { user_name, user_email, user_mobile, user_state_id, user_district_id, user_status, designation_code, user_roles, activity_group_code, diet_id, updated_by, assigned_group_activity_id };
        ...
        if (+result.user_role_id === 16) {
          if (request.data && request.data.user_roles && request.data.user_roles != result.user_roles) {
            ...
            if (idsArray.length > 0) {
              ...
              let deleteQuery = `delete from activity_group_location_mapping where user_role_id not in ${where} and user_id = ${request.id}`;
              const grpMappingResult = await Model.raw(deleteQuery);
            }
          }

          const iteratorForArray = Object.keys(request.data).filter((x) => x.includes("actgrpref"));

          let insertQueryString = `insert into activity_group_location_mapping (id, user_id, activity_group_code, location_group_code, user_role_id) values`;
          let valuesString = ``;
          let insertConflictString = ` on conflict(id) do update set activity_group_code = excluded.activity_group_code, location_group_code = excluded.location_group_code`;
          ...
          iteratorForArray.forEach((item, index) => {
            ...
            if (item == 'scactgrpref') {
              dbObj = {
                user_role_id: 15,
                activity_group_code: request.data[item],
                location_group_code: request.data['sclocgrpref'],
                user_id: request.id,
              ...
              dbObj = {
                user_role_id: item.startsWith("nfc") ? 2 : 3,
                activity_group_code: item.startsWith("nfc") ? request.data[`nfcactgrpref${item.charAt(item.length - 1)}`] : request.data[`nicactgrpref${item.charAt(item.length - 1)}`],
                location_group_code: item.startsWith("nfc") ? request.data[`nfclocgrpref${item.charAt(item.length - 1)}`] : request.data[`niclocgrpref${item.charAt(item.length - 1)}`],
                user_id: request.id,
                id: request.data[item.startsWith("nfc") ? `consultantnfc${item.charAt(item.length - 1)}id` : `consultantnic${item.charAt(item.length - 1)}id`] || "default",
              };
            }

            valuesString += `(${dbObj.id}, ${dbObj.user_id}, '${dbObj.activity_group_code}', '${dbObj.location_group_code}', '${dbObj.user_role_id}')`;
            valuesString += index + 1 == iteratorForArray.length ? `` : `,`;
          });

          if (valuesString) {
            const grpMappingResult = await Model.raw(insertQueryString + valuesString + insertConflictString);
          }
        }

        if (+result.user_role_id === 17 || +result.user_role_id === 23 || +result.user_role_id === 24) {
          if (request.data && request.data.user_roles && request.data.user_roles != result.user_roles) {
            //Delete data for updated user from consultant mapping
            let idsArray = request.data.user_roles.split(',').map(Number);
          ...
          if (valuesString) {
            ...
            console.log("consultantMappingResult==>", consultantMappingResult, insertQueryString + valuesString + insertConflictString);
          }
        }
...
exports.findById = async (req, res) => {
  ...
  try {
    ...
    if (role == 16) {
      data.consultantMappingData = await Model.findGrpData({ user_id: id })
    }

    if (role == 17 || role == 23 || role == 24) {
      // data.consultant;
    ...
    responseFromApiLogger = await ApiLog.create(token, req.originalUrl, "findById", req.body, response);
...
exports.findUserByRole = async (req, res) => {
  ...
  try {
    ...
    responseFromApiLogger = await ApiLog.create(token, req.originalUrl, "findUserByRole", req.body, response);
    res.status(200).json(response);
  } catch (e) {
...
exports.dashboard = async (req, res) => {
  ...
  try {
    ...
    const select = `select pmc.*, pmc.title as major_component_name, aa.financial_amount_recuring, aa.financial_amount_nonrecuring,
    case
    ...
    let query = `from
    (select sum(financial_amount) filter(where pd.recuring_nonrecuring = 1) as financial_amount_recuring,
      sum(financial_amount) filter(where pd.recuring_nonrecuring = 2) as financial_amount_nonrecuring, pd.scheme_id, pd.major_component_id
    from prb_ann_wrk_pln_bdgt_data pawpbd, public.prb_data pd
    ...
    group by pd.scheme_id, pd.major_component_id) aa
    left join prb_major_component pmc on (aa.scheme_id::numeric = pmc.scheme_id and aa.major_component_id = pmc.prb_major_component_id)`;

    const countResult = await Models.knx().raw(`SELECT COUNT(*) AS total_count ${query}`);
    const totalCount = countResult.rows[0].total_count;

    const object = await Models.knx().raw(`${select} ${query} limit ${limit} offset ${limit * (page - 1)}`);
...
exports.loggedUsersList = async (req, res) => {
  ...
  try {
    ...
    let object = await Model.query().leftJoin("master_designations as md", function () {
        this.on(Model.knx().raw("CAST(md.id AS varchar)"), "=", "u.designation_code");
      })
