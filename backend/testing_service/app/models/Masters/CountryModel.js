...
exports.all     = async (request) => await DB(table).select().orderBy('country_name').limit(request.limit).offset(request.limit*(request.page-1));

exports.list    = async () => await DB(table).select('country_code as id','country_name as name').orderBy('country_name');
