...
exports.findData = async (object = {}, limit, page) => {
    let data = await DB.raw(`SELECT "mb".*, "mtd"."atribute_name","mt"."title" as master_type_id, "md"."district_name", "ms"."state_name", "bk"."block_name"
    ...
    ORDER BY "mb"."created_at" DESC limit ${limit} offset ${limit * (page - 1)}`)
   
    return data.rows;
}

exports.findOnes = async object => (await DB(table).where(object).select())[0];
exports.findOne = async (object = {}) => {
    let data = await DB.raw(`SELECT "mb".*, "mtd"."atribute_name","mtd"."master_type_detail_id" ,"mt"."id" as master_type_id, "md"."district_name", "ms"."state_name", "bk"."block_name"
    FROM "master_common_data" AS "mb"
    INNER JOIN "master_districts" AS "md" ON "md"."id" = "mb"."district_id"
    INNER JOIN "master_states" AS "ms" ON "ms"."id" = "mb"."state_id"
    INNER JOIN "master_type_detail" AS "mtd" ON CAST("mtd"."master_type_detail_id" AS INTEGER) = CAST("mb"."master_type_detail_id" AS INTEGER)
