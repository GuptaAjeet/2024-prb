...
const table     = 'master_close_reasons';

exports.query   = () => DB(table);

exports.all     = async (request) => await DB(table).select().orderBy('reasons_name').limit(request.limit).offset(request.limit*(request.page-1));

exports.list    = async () => await DB(table).select('id','reasons_name').orderBy('reasons_name');
