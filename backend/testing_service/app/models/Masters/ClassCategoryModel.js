...
const table     = 'master_class_categories';

exports.query   = () => DB(table);

exports.all     = async (request) => await DB(table).select('id','class_category_name').orderBy('class_category_name').limit(request.limit).offset(request.limit*(request.page-1));

exports.list    = async () => await DB(table).select('id','class_category_name as name').orderBy('class_category_name');
