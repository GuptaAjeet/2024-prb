...
exports.all     = async (request) => await DB(table).select().orderBy('school_category_name').limit(request.limit).offset(request.limit*(request.page-1));

exports.list    = async () => await DB(table).select('id','school_category_name as name').orderBy('school_category_order');
