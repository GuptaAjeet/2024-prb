...
exports.all     = async (request) => await DB(table).select().orderBy('state_name').limit(request.limit).offset(request.limit*(request.page-1));

exports.list    = async () => await DB(table).select('id','state_name as name').orderBy('state_name');
