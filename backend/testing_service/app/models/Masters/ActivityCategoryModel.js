...
exports.all     = async () => await DB(table).select().orderBy('activity_category_name');

exports.list    = async () => await DB(table).select('id','activity_category_name as name','sub_cat_count as count').where({'activity_category_status':1}).orderBy('activity_category_name');
