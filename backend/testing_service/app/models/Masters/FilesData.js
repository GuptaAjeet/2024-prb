...
exports.findst = async object => await DB(table).where({ role_id: 4 } && { role_id: 8 }).select();
exports.finddata = async (object = {}) => {
    ...
    let data = await DB.raw(` select fd.*, "md"."district_name" ,"ms"."state_name" from files_data fd 
    LEFT JOIN "master_districts" AS "md" ON "md"."id" = "fd"."district_id"
    LEFT JOIN "master_states" AS "ms" ON "ms"."id" = "fd"."state_id"
    where is_private = 0 
    or is_private = 1 and (role_id >= ${object?.role_id} or user_id = ${object?.user_id})  ${state} ${district}
     ORDER BY "fd"."id" DESC 
