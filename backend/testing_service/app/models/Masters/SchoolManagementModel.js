...
const table     = 'master_school_managements';

exports.query   = () => DB(table);

exports.all     = async (request) => await DB(table).select().orderBy('school_management_name').limit(request.limit).offset(request.limit*(request.page-1));

exports.list    = async () => await DB(table).select('id','school_management_name as name').where('school_management_status',1).orderBy('school_management_name');
