...
exports.all     = async (request) => await DB(table).select().orderBy('specialization_name').limit(request.limit).offset(request.limit*(request.page-1));

exports.list    = async () => await DB(table).select('id','specialization_name as name').orderBy('specialization_order');
