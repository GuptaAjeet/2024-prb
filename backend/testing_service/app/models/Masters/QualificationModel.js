...
exports.all     = async (request) => await DB(table).select().orderBy('qualification_name').limit(request.limit).offset(request.limit*(request.page-1));

exports.list    = async () => await DB(table).select('id','qualification_name as name').orderBy('qualification_name');
