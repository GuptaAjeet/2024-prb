...
const table     = 'master_school_types';

exports.query   = () => DB(table);

exports.all     = async (request) => await DB(table).select().orderBy('school_type_name').limit(request.limit).offset(request.limit*(request.page-1));

exports.list    = async () => await DB(table).select('id','school_type_name as name').orderBy('school_type_name');
