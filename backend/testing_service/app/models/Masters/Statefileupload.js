...
exports.findCount = async (object = {}) => {
    ...
    const count = await DB.raw(`select count(*) as no_of_record from prb_state_documents psd 
...
exports.findData = async (object = {}) => {
    let data = await DB.raw(`SELECT psds.id  ,psds.udise_code  , psm.school_name , psm.district_name , psds.physical_quantity , psds.financial_amount,psds.activity_master_details_id,psds.status, psds.valid_assset
...
exports.findProposeDetail = async (object = {}) => {
    let data = await DB.raw(`select  psawpbd.proposed_physical_quantity , proposed_financial_amount 
...
exports.bulkDelete = async object => await DB(table).whereIn('id', object).delete()
