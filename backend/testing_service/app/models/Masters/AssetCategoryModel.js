...
const table     = 'master_asset_categories as mac';

exports.query   = () => DB(table);

exports.all     = async () => await DB(table).select().orderBy('asset_category_name');

exports.list    = async () => await DB(table).select('id','asset_category_name as name','sub_cat_count as count').where({'asset_category_status':1}).orderBy('asset_category_name');
