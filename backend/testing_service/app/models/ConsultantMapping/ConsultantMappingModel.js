...
exports.all     = async (request) => await DB(table).select().orderBy('id').limit(request.limit).offset(request.limit*(request.page-1));

exports.list    = async () => await DB(table).select('id','role_code as role', 'state_ids as state', 'consultant_task_ids as tasks', 'user_id').orderBy('id');
...
exports.getStates = async object => await DB(table).select('state_ids').where('cu.role_code', object)