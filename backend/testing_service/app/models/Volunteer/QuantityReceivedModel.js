...
const table     = 'quantity_received_collections as qrc';
...
exports.isEmailExist  = async object => {
    return ((await DB(table).where({volunteer_email:object.email}).whereNot({id:object.id}).count('id'))[0]).count > 0 ? true :false;
    // var Query  = DB(table);
...
exports.isMobileExist  = async object => {
    return ((await DB(table).where({volunteer_mobile:object.mobile}).whereNot({id:object.id}).count('id'))[0]).count > 0 ? true :false;
    // var Query  = DB(table);
    // var Query  = (object.flag) ? Query.where({role_id:object.role}) : Query.whereNot({id:object.id});
