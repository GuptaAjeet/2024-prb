...
const RecommendationDetail = () => {
  ...
  let repo_head = [
    {
      headerName: `Recommendation Detail ( ${stateList?.find((c) => +c.id === +stateID)?.name} )`,
      ...
      object?.data?.fields?.forEach((itm) => {
        ...
        if (Settings.isDataExistsInArray(itm?.name, "id")) {
          ...
          rowObj["valueGetter"] = (params) => {
            ...
            } else if (Settings.isDataExistsInArray(["physical_quantity"], itm.name)) {
  ...
  const exportDetailsTableToPDFs = async () => {
    ...
    try {
      ...
      link.setAttribute(
        "download",
        `Recommendation_Detail_${apiYear + "_" + currentdate}.pdf`
    ...
    <div className="dashboard-main-content">
      <div className="dashboard-main-content__header mb-3">
        ...
        <button
          ...
          onClick={exportDetailsTableToPDFs}
        >
          {pdfbtnStatus ? <Btnloader /> : ""}{" "}
          <i className="bi bi-file-earmark-pdf"></i>{" "}
          <span className="mobile-hide">Export To </span> PDF Details
        </button>
      </div>
