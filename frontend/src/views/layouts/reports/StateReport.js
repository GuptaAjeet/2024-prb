...
const StateReports = () => {
  ...
  const getStateMatrixData = () => {
    setSpin(true);
    api.post(
      "api/reports/state-costing-sheet-report",
      { state_id: stateID },
      (res) => {
        setObject(res);
        setSpin(false);
  ...
  let repo_head = [
    {
      headerName: `Costing Report ( ${state_name?.length > 0 && state_name[0]?.name} )`,
  ...
  const exportTableToExcel = async () => {
    ...
    exportToExcel("stateReport", {
      ...
      sheetName: "Sheet 1",
      report_header: `Costing Report: ${state_name?.length > 0 && state_name[0]?.name
        }, Year: ${apiYear}`,
    });
  };
