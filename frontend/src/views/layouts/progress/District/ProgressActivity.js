...
const ProgressActivity = () => {
  ...
  const [districtData, setDistrictData] = useState({ id: 0, name: "" });
  ...
  useEffect(() => {
    ...
    fetchSavedMonth(user?.user_state_id || 0);
  }, []);

  const districtInformation = () => {
    const userRolesArrary = user?.user_roles?.split(",").map(Number);
    const userRoleId = user?.user_role_id;
    const userDistrictId = user.user_district_id;
    const userDistrictName = user.district_name;
    if (userRolesArrary?.includes(userRoleId)) {
      setDistrictData({
        ...districtData,
        id: userDistrictId,
  ...
  const fetchSavedMonth = (state) => {
    API.post("api/prabandh/progress-active-month-data", { state: state },
      (res) => {
        if (res.data.length === 1) {
          dispatch(features.setMonth({ month: res.data[0].month_id }));
          localStorage.setItem("progress_month", res.data[0].month_id);
        } else {
          dispatch(features.setMonth({ month: 1 }));
    ...
    if (backState !== null) {
      API.post("api/prabandh/major-components", { schemeid: backState?.backParams?.activityDetails?.scheme_id },
        (res) => {
          ...
          API.post("api/prabandh/allocation-sub-components-list-district",
            {
              ...
              district_id: backState?.backParams?.filter?.district,
            },
            (res) => {
              setSubCompList(res.data);
              API.post("api/prabandh/allocation_activity-master-list-district",
                {
                  schemeid: backState?.backParams?.activityDetails?.scheme_id,
                  major_component_id: backState?.backParams?.activityDetails?.major_component_id,
  ...
  const handleAccordianBody = (activity) => {
    setSpin(true);
    API.post("api/prabandh/allocation_activity-master-list-details-district",
      {
        schemeid: filter?.scheme_id,
        major_component_id: filter?.major_component_id,
      ...
      <div className="dashboard-main-content-info mb-3 mt-3 app-bg" id="search">
        <div className="row mt-3">
          <div className="col-md-4">
            <select className="form-select" name="state" value={filter.state} onChange={(e) => handleChange(e)}
          ...
          <div className="col-md-4">
            <select className="form-select" name="district" defaultValue={user.user_district_id}
            // onChange={(e) => handleChange(e)}
            >
              <option value={user.user_district_id}>{user.district_name}</option>
            </select>
          </div>
        </div>
        ...
        <div className="row mt-3">
          <div className="col-md-4">
            <select className="form-select" name="scheme_id" value={filter?.scheme_id}
              onChange={(e) => {
                handleChange(e);
                            ...
                            {actMasterDetailList?.length > 0 && actMasterDetailList[0].activity_master_id === activity.id &&
                              actMasterDetailList.map((d, didx) => (
                                <li key={`${index}${didx}`} className="list-group-item" style={{ cursor: "pointer" }}>
                                  <Link to={`/auth/progress/district/${d.dd_school === "1" ? "tracking-school-activity" : "tracking"}`}
                                    className="text-black"
                                    state={{
                                      activity: activity,
                                      activityDetails: d,
                                      filter: filter,
                                      state_name: stateList?.filter((v) => +v.id === +filter?.state)[0]?.name,
                                    }}
                                  >
