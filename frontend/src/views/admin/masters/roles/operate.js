...
const Operate = (props) => {    
    ...
    const object = Hook.usePost({ url: "api/roles/find", data: { 'id': props.id } });
   
    useEffect(()=>{            
        if(object !== null && object.data[0] !== undefined) {
            name.current.value =  object.data[0].role_name;
        }
    },[object]) 
    ...
    const createHandler = (e) => {
        ...
        if(isvalid){
            ...
            if (props.btntext === 'Add') {
                var url = 'create';
                var data = { 'role_name': name.current.value };
            } else {
                var url = 'update';
                var data = { id: props.id, 'role_name': name.current.value };
            }
            API.post(`api/roles/${url}`, data, (response) => {
