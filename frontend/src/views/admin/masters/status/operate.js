...
const Operate = (props) => {
    ...
    const object = Hook.usePost({ url: "api/status/find", data: { 'id': props.id } });

    useEffect(() => {
        if (object !== null && object.data !== undefined) {
            name.current.value = object.data.status_name;
        }
    }, [object])
    ...
    const createHandler = (e) => {
        ...
        if (isvalid) {
            ...
            if (props.btntext === 'Add') {
                var url = 'create';
                var data = { 'status_name': name.current.value };
            } else {
                var url = 'update';
                var data = { id: props.id, 'status_name': name.current.value };
            }

            API.post(`api/status/${url}`, data, (response) => {
        ...
        <Modal clickHandler={createHandler}>
            <div className="row p-3">
                <div className="col-md-12 mb-4">
                    <Form.Text attr={{ ref: name, 'id': 'name', 'name': 'name', 'onChange': status, 'onFocus': status, 'maxLength': 100 }} label="Name" error={error.name} mandatory="true" />
                </div>
